...
public class HW4WorkerTest extends GenericTest {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if ((args.length == 0) || args[0].equals("all") || args[0].equals("auto1")) {
      ...
      tests.add("overwrite");
      tests.add("stress");
    } else if (args[0].equals("auto2")) {
      tests.add("newid");
      tests.add("writeid");
    }

    for (int i = 0; i < args.length; i++) {
      if (!args[i].equals("all") && !args[i].equals("auto1") && !args[i].equals("auto2")
          && !args[i].equals("setup1") && !args[i].equals("setup2") && !args[i].equals("cleanup1")
          && !args[i].equals("cleanup2")) {
        tests.add(args[i]);
      }
    }
  ...
  void prompt() {
    ...
    System.out.println(
        "and then hit Enter in this window to continue. If the Master and/or the Worker are already running, please terminate them and restart the test suite!");
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("read-id")) {
      try {
        ...
        if (r.statusCode != 200) {
          testFailed("The master returned a " + r.statusCode
        ...
        if (!pieces[0].equals("1")) {
          testFailed(
              "The master did return a 200 status code to our GET /workers, but it was supposed to return a '1' in the first line, and it didn't. Here is what was in the body:\n\n"
                  + dump(r.body)
                  + "\nMaybe the worker didn't register properly? Check the ping thread!");
        }
        if ((pieces.length < 2) || !pieces[1].contains(id)) {
          testFailed(
              "The master did return one worker in its response to our GET /workers, but the ID we had written to __worker/id ("
                  + id + ") did not appear. Here is what the master sent:\n\n" + dump(r.body)
    ...
    if (tests.contains("put")) {
      try {
        ...
        String req =
            "PUT /data/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(
                5, 8);
        out.print(
    ...
    if (tests.contains("putget")) {
      try {
        ...
        PrintWriter out = new PrintWriter(s.getOutputStream());
        String cell =
            "/data/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5, 8) + "/" + randomAlphaNum(5,
                8);
                ...
                try {
                  ...
                  while (testsRemaining.decrement()) {
                    ...
                    if (random(0, 1) == 0) {
                      ...
                      out.print(
                          "GET /data/test/" + row + "/col HTTP/1.1\r\nHost: localhost\r\n\r\n");
                      out.flush();
                      Response r = readAndCheckResponse(s, "response");
                      if (expectedAtLeastVersion > 0) {
                        if (r.statusCode != 200) {
                          ...
                          System.err.println(
                              "GET returned code " + r.statusCode + " for row: " + row + ", body: "
                                  + new String(r.body));
                        } else {
                          ...
                          if (!dat.get(row + "-" + v).equals(val)) {
                            ...
                            System.err.println("GET returned value " + val + " for row " + row
                                + ", but we expected version " + v + " to be " + dat.get(
                                row + "-" + v).equals(val));
                          }
                        }
                      }
                      ...
                      if (r.statusCode != 200) {
                        testsFailed.increment();
                        System.err.println(
                            "PUT returned code " + r.statusCode + " for row: " + row + ", body: "
                                + new String(r.body));
                      }
                    }
                  }
    ...
    if (stressRan) {
      System.out.println(
          "\nYour throughput in the stress test was " + reqPerSec + " requests/sec, with " + numFail
              + " failues");
    }
    cleanup();
    closeOutputFile();
